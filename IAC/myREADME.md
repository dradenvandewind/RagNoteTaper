...
└── data EBS 120 GB  (ChromaDB + HF models + Ollama weights)
        mounted → /data
        ├── chroma_db/
        ├── hf_cache/
        └── ollama_data/
...
ssh -R 1080:localhost:1080 ubuntu@<ec2-ip>


on other shell ssh -D 1080 -N localhost


test tunnel on ec2 

                                                                                                  0.2s
ubuntu@ip-10-0-1-146:/opt/rag-llamaindex$ curl --socks5 localhost:1080 https://www.youtube.com -I



curl -s ifconfig.me

https://ipinfo.io/ec2_ip

Tu peux configurer yt-dlp pour qu'il passe par un proxy qui utilise une adresse IP résidentielle (qui ressemble à celle d'une box internet classique) :